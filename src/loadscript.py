...
def load_dim_customer(parquet_path: str) -> int:
    ...
    sql = """
        INSERT INTO dim_customer (customer_id, customer_unique_id, zip_code_prefix)
        VALUES (%s, %s, %s)
    ...
    try:
        with conn.cursor() as cur:
            psycopg2.extras.execute_batch(
                cur, sql, df_to_records(df), page_size=1000,
            )
        conn.commit()
    except Exception:
        conn.rollback()
...
def load_dim_seller(parquet_path: str) -> int:
    ...
    sql = """
        INSERT INTO dim_seller (seller_id, zip_code_prefix)
        VALUES (%s, %s)
        ...
        with conn.cursor() as cur:
            ...
            cur.execute("""
                INSERT INTO fact_order (
                    ...
                    customer_location_key, seller_location_key,
                ...
                LEFT JOIN dim_location cl  ON cl.zip_code_prefix  = s.customer_zip_code_prefix
                LEFT JOIN dim_location sll ON sll.zip_code_prefix = s.seller_zip_code_prefix
