...
CHECKS = [
    ...
    (
        "no NULL seller_key",
        "SELECT COUNT(*) FROM fact_order WHERE seller_key IS NULL",
        lambda x: x == 0,
    ),
    (
        "no NULL order_date_key",
        "SELECT COUNT(*) FROM fact_order WHERE order_date_key IS NULL",
        lambda x: x == 0,
    ),
    (
        ...
        LEFT JOIN dim_location l
            ON f.customer_location_key = l.location_key
        WHERE f.customer_location_key IS NOT NULL
    ...
    (
        ...
        FROM fact_order f
        LEFT JOIN dim_location l
            ON f.seller_location_key = l.location_key
        WHERE f.seller_location_key IS NOT NULL
          AND l.location_key IS NULL
        """,
        lambda x: x == 0,
    ),
    (
        "no negative prices",
        "SELECT COUNT(*) FROM fact_order WHERE price < 0",
        lambda x: x == 0,
    ),
    (
        "no negative freight",
        "SELECT COUNT(*) FROM fact_order WHERE freight_value < 0",
        lambda x: x == 0,
    ),
    (
        "review scores 1–5",
